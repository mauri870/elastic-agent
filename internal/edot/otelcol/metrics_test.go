...
func TestMetricsTelemetryStartingEvent(t *testing.T) {
	tests := []struct {
		name                string
		logEventStatus      int
	...
	}{
		{
			...
			description:         "All events should be successfully processed and acked",
		},
		{
			name:                "failure",
			logEventStatus:      http.StatusInternalServerError,
			...
			expectedBatches:     2,
			expectFailedField:   false,
			description:         "Events should still be acked despite ES returning errors",
		},
		{
			name:                "retry_exhausted_events_failed",
			logEventStatus:      http.StatusTooManyRequests, // 429 - retryable but will exhaust retries
			...
			expectFailedField:   false,
			description:         "Log events should be failed after exceeding max retries",
		},
	}
