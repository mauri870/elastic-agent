...
func TestMonitoringTelemetryMetricsFailed(t *testing.T) {
	cfg := `receivers:
...
`
	...
	expected := mapstr.M{
		...
		"beat.stats.libbeat.output.events.batches":        float64(8),
		"beat.stats.libbeat.output.events.failed":         float64(4),
...
func TestMonitoringTelemetryMetricsDropped(t *testing.T) {
	cfg := `receivers:
  elasticmonitoringreceiver:
    interval: 1s
exporters:
